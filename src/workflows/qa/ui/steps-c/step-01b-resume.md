...
description: 'Resume an interrupted UI testing workflow'
nextStepFile: null
...
2. **Detected configuration** — UI framework, browser automation tool, etc.
3. **Generated files list** — Which files were already created
4. **Any blockers** — Issues noted in the progress file

### 1B.2 — Verify Artifacts

For each completed step, verify the expected output files still exist:

- **Step 1 complete:** Check that page inventory and configuration detection are recorded
- **Step 2 complete:** Check that test plan with risk matrix exists
- **Step 3 complete:** Check that config file, base page object, auth helper, and directory structure exist
- **Step 4 complete:** Check that test spec files and page objects exist
- **Step 5 complete:** Check that visual and a11y test files exist
...
|---------------|--------------------------------|
| Step 1        | `./step-02-design-test-plan.md` |
| Step 2        | `./step-03-setup-framework.md`  |
| Step 3        | `./step-04-generate-tests.md`   |
| Step 4        | `./step-05-visual-a11y.md`      |
